...
class RandomIndexing(object):
    ...
    ##
    def __init__(self, filenames, dimension=2000, non_zero=100, non_zero_values=list([-1, 1]), left_window_size=1, right_window_size=3):
    ...
    def create_word_vectors(self):
        ...
        for word in self.__vocab:
            ...
            RandVectorIndex=random.sample(range(self.__dim),self.__non_zero)
            temp_np_list=[0]*self.__dim
            for i in RandVectorIndex:
                temp_np_list[i]=random.choice(self.__non_zero_values)
            self.__rv[word]=np.array(temp_np_list)
            ...
            for word in word_list:
                ...
                for j in range(1,self.__rws+1):
                        if(i+j)<no_of_words:
                            ...
                            self.__cv[word]=self.__cv.get(word)+self.__rv.get(word_list[i+j])           
    ...
    def find_nearest(self, words, k=5, metric='cosine'):
        ...
        for word in words:
            ...
            dist_list, indexes=neigh.kneighbors([self.get_word_vector(word)],return_distance=True)
            close_words=list()
